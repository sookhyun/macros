		       ...
		       )
{
 ...
 // gSystem->Load("libfun4allfuncs.so");
  ...
  //---------------
     ...
     PHOnlineDisplay* online_display
        = new PHOnlineDisplay(1920,860,true,"sPHENIX.2d.root", "sphenix_maps+tpc_geo.root",
				5.,20.,0.2,true,false,true);
		/*jet_pt_threshold, jet_e_scale, calo_e_threshold, is_svtx_on, is_calo_on, is_jet_on */

  //----------------------------
  // Link the display to Fun4All
  //----------------------------
  se->registerSubsystem(online_display);
  //online_display->start_rotation();
  ...
  timer = new TTimer();
  timer->Connect("Timeout()", "PHOnlineDisplay", online_display,"run_evt_in_thread()");
